...
export class Seasons {
  ...
  public static async getFromId(id: number) {
    return await getDB().selectFrom("seasons").selectAll().where("id", "=", id).executeTakeFirst();
  }
  ...
  public static async getFromTimestamp(timestamp: string): Promise<Season[]> {
    const sql = getDB()
      ...
      .selectAll()
      .where((eb) =>
        eb("started_at", "<=", timestamp).and(eb.parens(eb("ended_at", ">=", timestamp).or(eb("ended_at", "is", null))))
      )
  ...
  public static async insert(user: InsertSeason): Promise<Season> {
    return await getDB().insertInto("seasons").values(user).returningAll().executeTakeFirst();
  }
}
